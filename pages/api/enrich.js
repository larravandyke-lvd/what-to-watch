...
export default async function handler(req, res) {
  ...
  try {
    ...
    const prompt = `Look up the title "${title}".${disambiguation} Determine if it's a Movie or TV Show. Respond with ONLY a raw JSON object, no markdown, no preamble, in this exact shape:
{"type":"Movie or TV Show","year": number or null,"service":"primary streaming service or network","genres":["genre1","genre2"],"cast":["actor1","actor2","actor3"],"rtScore": number or null,"rtLink":"url to the Rotten Tomatoes page or null","synopsis":"one sentence"}
